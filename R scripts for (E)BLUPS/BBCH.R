...
# Loading Data
df <- read.csv2("BBCH.csv", stringsAsFactors = TRUE)

#Wide to Long
df_long <- df %>%
  pivot_longer(
    cols = c(BBCH1, BBCH2),  # column names
    names_to = "Rep",               # new column for rep
    values_to = "BBCH"               # new column for trait value
  )
...
model_fixed <- lmer(BBCH ~ Genotype + (1|Rep), data = df_long)
...
model_blup <- lmer(BBCH ~ 1 + (1|Genotype) + (1|Rep), data = df_long)
